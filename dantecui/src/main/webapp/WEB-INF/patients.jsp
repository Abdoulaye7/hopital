...
<div class="container mt-4">
    ...
    <a href="<c:url value='/formulairePatient'/>" class="btn btn-primary mt-3"><i class="fas fa-plus"></i> Ajouter Patient</a>
    ...
    <table class="table table-hover mt-3">
      ...
      <tbody>
        <c:forEach var="patient" items="${listePatients}" varStatus="loop">
              <tr class="table-light">
                    ...
                    <td>${patient.prenom}</td>
                     <td>${patient.dateNaissance}</td>
                      ...
                      <td>
                        <a href="<c:url value='/modifierPatient'/>?id=${patient.id}" class="btn btn-primary"><i class="fas fa-edit"></i> Modifier</a>

                          <form action="<c:url value='/supprimerPatient'/>" method="post" style="display: inline;">
                             <input type="hidden" name="id" value="${patient.id}" />
                             <button type="submit" class="btn btn-danger" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce patient ?')"><i class="fas fa-trash"></i> Supprimer</button>
                          </form>
                      </td>
          </tr>
         </c:forEach>
